...
function validator(note) {
  const errors = [];
  const detail = {note};
  const noteError = new NotesError('NOTE_FAILED_VALIDATION', detail);
  ...
  if (!isValidUuid(note.id)) {
    ...
    detail.dbIsLikelyCorrupt = true;
  }
  if (errors.length > 0) {
    noteError.message = `suggested note does not validate:${errors.toString()}`;
    throw noteError;
  }
}
